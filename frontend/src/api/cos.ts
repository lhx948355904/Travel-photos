...
const createUploadConfig = (options: UploadOptions) => ({
  ...
  onUploadProgress: (event: any) => {
    ...
    const percent = Math.min(80, Math.max(1, Math.round((event.loaded / event.total) * 80)))
...
export const uploadToCos = (file: File, options: UploadOptions = {}): Promise<CosUploadResult> => {
  const config = createUploadConfig(options)
  const postUpload = (url: string) => {
    return request.post(url, createUploadFormData(file), config) as Promise<CosUploadResult>
  }
