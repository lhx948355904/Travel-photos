...
const collectBackgroundImages = (locations: Location[]) => {
  ...
  return Array.from(new Set(imageUrls.filter((url): url is string => Boolean(url)))).slice(0, 18)
}

const Landing = () => {
  const navigate = useNavigate()
  const [backgroundImages, setBackgroundImages] = useState<string[]>(fallbackBackgroundImages)
  ...
  useEffect(() => {
    ...
    getLocations()
      .then((locations) => {
        ...
        setBackgroundImages(uploadedImages.length > 0 ? uploadedImages : fallbackBackgroundImages)
      })
      .catch(() => {
